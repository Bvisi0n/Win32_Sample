#ifndef DATEPICKERMODULE_H
#define DATEPICKERMODULE_H
...
public:
    ...
    [[nodiscard]] int Initialize(HWND ownerHandle, float dpiScale, int yPosition) override
    {
        ...
        m_DatePickerHandle = CreateWindowEx(
            ...
            ID::ToHandle(ID::DatePickerModule::DatePicker),     // Control ID
    ...
    {
        if (!m_ParentHandle || !m_DatePickerHandle)
        {
        ...
        else
        {
            ...
            int availableWidth{ parentWidth - (2 * margin) };

            MoveWindow(m_DatePickerHandle,  // Handle
                margin,                     // X position
...
private:
    HWND m_DatePickerHandle = nullptr;
};
